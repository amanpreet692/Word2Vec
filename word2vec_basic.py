...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  """
    ...
    while current_batch_size < batch_size:
        curr_num_skip = 0
        for skip_index in range((data_index - skip_window), (data_index + skip_window + 1)):
            if skip_index == data_index or skip_index < 0 or skip_index > len(data):
                continue
            else:
                labels[current_batch_size][0] = data[skip_index]
                batch[current_batch_size] = data[data_index]
                current_batch_size = current_batch_size + 1
                curr_num_skip = curr_num_skip + 1

                if curr_num_skip == num_skips or current_batch_size == batch_size:
                    break
...
if __name__ == '__main__':
    ...
    # [dictionary['first'], dictionary['american'], dictionary['would']]
